          ...
          <div class="card">
            ...
            <div class="card-body">
              <table id="example1" class="table table-bordered table-striped">
                ...
                <td>
                  <a title="view"  id="view" href="{{route('color.show',$item->id)}}" data-id="{{$item->id}}"><i class="btn btn-sm btn-success fa fa-eye"></i></a>
                  <a title="edit" id="edit" href="{{route('color.edit',$item->id)}}" data-id="{{$item->id}}"><i class="btn btn-sm btn-primary fas fa-edit" aria-hidden="true"></i></a>
                  {{-- <a title="delete"  id="delete" onclick="return confirm('Are sure to delete this ?')"  href="" data-id="{{$item->id}}"></a> --}}
                  <button type="button" class="btn btn-sm btn-danger" name="button" onclick="Color({{$item->id}})"><i class="fa fa-trash fa-lg"></i></button>
                    <form id="color-{{$item->id}}" action="{{route('color.destroy',$item->id)}}" method="post">
                      @csrf
                      @method('Delete')
                    </form>
                </td>
...
document.getElementById('color-'+id).submit();
} else if (
